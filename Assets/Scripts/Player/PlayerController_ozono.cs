...
namespace Metroidvania.Player
{
    ...
    public sealed class PlayerController : MonoBehaviour
    {
        ...
        private float CurrentMoveSpeed => statsData != null ? statsData.MoveSpeed : 6f;

        /// <summary>滑空中の移動速度</summary>
        private float CurrentGlideMoveSpeed => statsData != null ? statsData.GlideMoveSpeed : 7f;

        /// <summary>ジャンプ時の上方向インパルス</summary>
        private float CurrentJumpImpulse => statsData != null ? statsData.JumpForce : 10f;

        /// <summary>通常時の最大落下速度</summary>
        private float CurrentMaxFallSpeed => statsData != null ? statsData.JumpForce : 12f;
        ...
        private void FixedUpdate()
        {
            ...
            // 滑空中は滑空専用速度、それ以外は通常移動速度を使う
            float currentMoveSpeed = IsGliding ? CurrentGlideMoveSpeed : CurrentMoveSpeed;

            velocity.x = _moveInputX * currentMoveSpeed;
            _rigidbody2D.linearVelocity = velocity;

            // 接地中にジャンプ入力があればジャンプする
            if (_jumpQueued && _isGrounded)
            {
                ...
                _rigidbody2D.AddForce(Vector2.up * CurrentJumpImpulse, ForceMode2D.Impulse);
            }
        ...
        {
            Vector2 velocity = _rigidbody2D.linearVelocity;
            float limit = ShouldGlide() ? glideMaxFallSpeed : CurrentMaxFallSpeed;
