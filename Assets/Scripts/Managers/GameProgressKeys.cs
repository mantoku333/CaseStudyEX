public static class GameProgressKeys
{
    public const string Boss01Defeated = "boss_01_defeated";
    public const string Boss02Defeated = "boss_02_defeated";
    public const string LastBossDefeated = "last_boss_defeated";
    public const string OpenedBlueDoor = "opened_blue_door";
    public const string TutorialCompleted = "tutorial_completed";
    public const string TutorialAttackShown = "tutorial_attack_shown";
    public const string TutorialDodgeShown = "tutorial_dodge_shown";
    public const string TutorialParryShown = "tutorial_parry_shown";
    public const string TutorialGimmickShown = "tutorial_gimmick_shown";
    public const string TutorialGlideShown = "tutorial_glide_shown";
    public const string TutorialGunShown = "tutorial_gun_shown";
    public const string TutorialBasicMoveShown = "tutorial_basicmove_shown";
    public const string PrologueStarted = "prologue_started";
    public const string PrologueCompleted = "prologue_completed";
    public const string PrologueCompletionUiShown = "prologue_completion_ui_shown";

    //--------------能力解放関連------------------
    public const string AbilityDodgeUnlocked = "ability_dodge_unlocked";
    public const string AbilityGlideUnlocked = "ability_glide_unlocked";
    public const string AbilityGunRecoilUnlocked = "ability_gun_recoil_unlocked";
    public const string AbilityParryUnlocked = "ability_parry_unlocked";

    //--------------デコレーション関連------------
    public const string EquipmentBlueAuraUnlocked = "Equipment_BlueAura_Unlock";
    public const string EquipmentRedAuraUnlocked = "Equipment_RedAura_Unlock";
    public const string EquipmentArcancielUnlocked = "Equipment_Arcanciel_Unlock";
}
