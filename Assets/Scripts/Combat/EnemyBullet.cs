...
namespace Metroidvania.Enemy
{
    ...
    public sealed class EnemyBullet : MonoBehaviour
    {
        ...
        private bool TryApplyPlayerHit(GameObject hitObject)
        {
            ...
            Debug.Log($"TryApplyPlayerHit確認: {hitObject.name} / Tag:{hitObject.tag}");

            if (!hitObject.CompareTag("Player"))
            {
                Debug.Log("Playerタグではないのでダメージなし");
                return false;
            }
            ...
            if (playerHealth == null)
            {
                Debug.LogError("PlayerタグはあるけどPlayerHealthが見つかりません", hitObject);
                return false;
            }

            Debug.Log($"Playerにダメージを入れます: {damage}");
